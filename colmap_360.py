...
def pipeline(scene, base_path, n_views):
    ...
    '--FeatureExtraction.use_gpu 1').read()
    os.system(
...
for scene in ['bicycle', 'bonsai', 'counter', 'garden', 'kitchen', 'room', 'stump']:
    for n_views in [12, 24]:
        pipeline(
            scene,
            base_path='/data0/anaconda3/wuyao/mipnerf360/',  # 请使用绝对路径
            n_views=n_views
        )
#for scene in ['bicycle', 'bonsai', 'counter', 'garden',  'kitchen', 'room', 'stump']:
 #   pipeline(scene, base_path = '/data0/anaconda3/wuyao/mipnerf360/', n_views = 24)  # please use absolute path!
